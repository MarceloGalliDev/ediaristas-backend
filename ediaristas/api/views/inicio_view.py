...
class Inicio(APIView):
  def get(self, request, format=None):
    links = Hateoas()
    links.add_get('listar_servicos', reverse('servico-list'))#estamos acessando pelo name
    ...
    links.add_post('login', reverse('logout-list')),
    links.add_get('usuario_logado', reverse('me-list')),
    return Response({"links": links.to_array()}, status=status_http.HTTP_200_OK)
  

#aqui usamos biblioteca do django para manusear as rotas, deixando mais segura as rotas de acesso